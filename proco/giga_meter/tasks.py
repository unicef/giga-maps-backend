...
def giga_meter_handle_published_school_master_data_row(*args, country_ids=None, force_tasks=False):
    ...
    if task_instance:
        ...
        try:
            ...
            logger.exception('Error during giga_meter_handle_published_school_master_data_row')
...
def giga_meter_handle_deleted_school_master_data_row(*args, country_ids=None, force_tasks=False):
    ...
    if task_instance:
        ...
        try:
            ...
            logger.exception('Error during giga_meter_handle_deleted_school_master_data_row')
            task_instance.error(f'Error occurred: {exc}')
            raise
        finally:
            background_task_utilities.task_on_complete(task_instance)
...
def giga_meter_update_static_data(*args, country_iso3_format=None, force_tasks=False):
    ...
    if task_instance:
        ...
        try:
            ...
            logger.exception('Error during giga_meter_update_static_data')
            task_instance.error(f'Error occurred: {exc}')
            raise
        finally:
            background_task_utilities.task_on_complete(task_instance)
    ...
    if settings.GIGA_METER_ENABLE_AUTO_SYNC:
        ...
        if task_instance:
            ...
            try:
                ...
                logger.exception('Error during handle_giga_meter_school_master_data_sync')
                task_instance.error(f'Error occurred: {exc}')
                raise
            finally:
                background_task_utilities.task_on_complete(task_instance)
...
def scheduler_for_populate_school_geopoint_field(country_iso3_format):
    ...
    if task_instance:
        ...
        try:
            ...
            logger.exception('Error during scheduler_for_populate_school_geopoint_field')
            task_instance.error(f'Error occurred: {exc}')
            raise
        finally:
            background_task_utilities.task_on_complete(task_instance)
...
def scheduler_for_backup_giga_meter_connectivity_ping_data(*args, start_date=None, end_date=None):
    ...
    if task_instance:
        ...
        try:
            ...
            logger.exception('Error during scheduler_for_backup_giga_meter_connectivity_ping_data')
            task_instance.error(f'Error occurred: {exc}')
            raise
        finally:
            background_task_utilities.task_on_complete(task_instance)
