...
def collect_data(country_id, entity_id):
    ...
    docs = []
    for qry_data in qs:
        qry_data['entity_id'] = str(qry_data['entity_id'])
...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        logger.info('Index operations STARTED ({0})'.format(EntityIndex.Meta.index_name))
        if settings.ENABLE_AZURE_COGNITIVE_SEARCH:
            ...
            if options.get('update_index', False):
                ...
                if country_id or entity_id:
                    data_to_load = collect_data(country_id, entity_id)
        ...
        logger.info('Index operations ENDED ({0})'.format(EntityIndex.Meta.index_name))
