...
class BrasilSimnetLoader(object):
    ...
    @cached_property
    def country(self):
        return Country.objects.filter(code='BR').first()
    ...
    def save_school(self, school_data):
        ...
        if required_fields - set(school_data.keys()) == set():
            ...
            if created:
                school_admin_updated = False
                admin1_name = school_data.get('NM_ESTADO', '')
                if not is_blank_string(admin1_name):
                    ...
                    if admin1_instance:
                        school.admin1 = admin1_instance
                        school_admin_updated = True

                admin2_name = school_data.get('NM_MUNICIP', '')
                if not is_blank_string(admin2_name):
                    ...
                    if admin2_instance:
                        school.admin2 = admin2_instance
                        school_admin_updated = True

                if school_admin_updated:
                    school.save()
