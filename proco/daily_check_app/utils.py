...
def sync_school_master_data(
    ...
    country = daily_check_app_models.DailyCheckApp_Country.objects.filter(iso3_format=table_name, ).first()
    ...
    country_latest_school_master_data_version = daily_check_app_models.DailyCheckApp_SchoolMasterData.get_last_version(table_name)
        ...
        for _, row in loaded_data_df.iterrows():
            ...
            if change_type in ['insert', 'update_postimage', 'remove', 'delete']:
                if change_type in ['remove', 'delete']:
                    row['status'] = daily_check_app_models.DailyCheckApp_SchoolMasterData.ROW_STATUS_DELETED

                row_as_dict = sources_utilities.parse_row(row)
                insert_entries.append(daily_check_app_models.DailyCheckApp_SchoolMasterData(**row_as_dict))

                if len(insert_entries) == 5000:
                    logger.debug('Loading the data to "SchoolMasterData" table as it has reached 5000 benchmark.')
                    daily_check_app_models.DailyCheckApp_SchoolMasterData.objects.bulk_create(insert_entries)
                    insert_entries = []
                    logger.debug('#' * 10)
                    logger.debug('\n\n')
    ...
    daily_check_app_models.DailyCheckApp_SchoolMasterData.set_last_version(table_current_version, table_name)
