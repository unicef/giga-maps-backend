...
from proco.daily_check_app import models as daily_check_app_models
from proco.daily_check_app import utils as daily_check_app_utilities
...
def daily_check_app_load_data_from_school_master_apis(country_iso3_format=None):
    """
    Background task which handles School Master Data source changes from APIs to DailyCheckApp DB

    Execution Frequency: Once in a day
    """
    logger.info('Starting loading the school master data from API to DailyCheckApp DB.')
    ...
    profile_file = os.path.join(
        settings.BASE_DIR,
        'daily_check_app_school_master_profile_{dt}.share'.format(
    ...
    if school_master_share:
        ...
        if school_master_schema:
            ...
            logger.debug('All tables ready to access for Daily Check App Sync: {0}'.format(schema_tables))

            school_master_fields = [f.name for f in daily_check_app_models.DailyCheckApp_SchoolMasterData._meta.get_fields()]
            ...
            for schema_table in schema_tables:
                ...
                try:
                    daily_check_app_utilities.sync_school_master_data(
...
def daily_check_app_handle_published_school_master_data_row(country_ids=None):
    ...
    """
    logger.info('Daily Check App - Handling the published school master data rows.')
    ...
    if task_instance:
        logger.debug('Not found running job for daily check app published rows handler task: {}'.format(task_key))

        new_published_records = daily_check_app_models.DailyCheckApp_SchoolMasterData.objects.filter(
            status=daily_check_app_models.DailyCheckApp_SchoolMasterData.ROW_STATUS_PUBLISHED,
        )

        if country_ids and len(country_ids) > 0:
            new_published_records = new_published_records.filter(country_id__in=country_ids)

        task_instance.info('Daily Check App - Total published records to update: {}'.format(new_published_records.count()))
        ...
        for data_chunk in core_utilities.queryset_iterator(new_published_records, chunk_size=100, print_msg=False):
            for row in data_chunk:
                try:
                    ...
                    school, created = daily_check_app_models.DailyCheckApp_School.objects.update_or_create(
                    ...
                    school_static, school_static_created = daily_check_app_models.DailyCheckApp_SchoolStatic.objects.update_or_create(
                        school=school,
                        version=row.version,
                        defaults={
...
def daily_check_app_handle_deleted_school_master_data_row(country_ids=None):
    ...
    if task_instance:
        ...
        new_deleted_records = daily_check_app_models.DailyCheckApp_SchoolMasterData.objects.filter(
            status=daily_check_app_models.DailyCheckApp_SchoolMasterData.ROW_STATUS_DELETED,
        )
        ...
        for data_chunk in core_utilities.queryset_iterator(new_deleted_records, chunk_size=1000):
            for row in data_chunk:
                try:
                    school = daily_check_app_models.DailyCheckApp_School.objects.filter(
                        giga_id_school=row.school_id_giga,
                        country=row.country,
                    ).first()
...
def daily_check_app_update_static_data(*args, country_iso3_format=None):
    ...
    if task_instance:
        ...
        daily_check_app_load_data_from_school_master_apis(country_iso3_format=country_iso3_format)
        ...
        rows_with_more_than_1_record = daily_check_app_models.DailyCheckApp_SchoolMasterData.objects.all().values(
        ...
        for row in rows_with_more_than_1_record:
            for row_to_delete in daily_check_app_models.DailyCheckApp_SchoolMasterData.objects.filter(
