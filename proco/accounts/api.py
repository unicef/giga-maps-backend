...
class DataLayerPreviewViewSet(APIView):
    ...
    def get_map_query(self, kwargs):
        ...
        ORDER BY schools_school.giga_id_school ASC
        LIMIT 1000
        """
    ...
    def get_live_map_query(self, env, request):
        ...
        if add_random_condition:
            if 'limit' in request.query_params:
                ...
                kwargs['random_order'] = 'ORDER BY schools_school.giga_id_school ASC' if int(request.query_params.get('z', '0')) == 2 else ''
            elif kwargs.get('MAP_API_SAMPLING_LIMIT'):
                limit = kwargs['MAP_API_SAMPLING_LIMIT']
                kwargs['random_order'] = 'ORDER BY schools_school.giga_id_school ASC'
            else:
                limit = '50000'
                kwargs['random_order'] = 'ORDER BY schools_school.giga_id_school ASC' if int(request.query_params.get('z', '0')) == 2 else ''

            kwargs['limit_condition'] = 'LIMIT ' + str(limit)
            kwargs['random_select_list'] = 'schools_school.giga_id_school ASC,'
